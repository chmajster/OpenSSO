...
func Migrate(ctx context.Context, p *pgxpool.Pool) error {
	...
	es, e := migrations.ReadDir("migrations")
	if e != nil {
		return e
	}
	names := []string{}
	...
	for _, n := range names {
		var done bool
		if e = p.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM schema_migrations WHERE version=$1)`, n).Scan(&done); e != nil {
			return e
		}
		if done {
			continue
		}
		sql, e := migrations.ReadFile("migrations/" + n)
		if e != nil {
			return e
		}
		tx, e := p.Begin(ctx)
		...
		if _, e = tx.Exec(ctx, `INSERT INTO schema_migrations(version) VALUES($1)`, n); e != nil {
			tx.Rollback(ctx)
			return e
		}
