...
func (s *Server) scimUsers(w http.ResponseWriter,r *http.Request){
	...
	w.Header().Set("Content-Type","application/scim+json");writeJSON(w,200,map[string]any{"schemas":[]string{"urn:ietf:params:scim:api:messages:2.0:ListResponse"},"totalResults":len(res),"startIndex":start,"itemsPerPage":len(res),"Resources":res})
}
